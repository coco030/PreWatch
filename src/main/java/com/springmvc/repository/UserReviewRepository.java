...
public class UserReviewRepository {
 ...
 // ✅ 유저가 평가한 호러 점수 평균 계산 (user_reviews 기준)
    ...
    public String findSignatureGenre(String memberId) {
        String sql = "SELECT g.genre FROM user_reviews r JOIN movie_genres g ON r.movie_id = g.movie_id WHERE r.member_id = ? AND r.user_rating IS NOT NULL GROUP BY g.genre ORDER BY COUNT(*) * AVG(r.user_rating) DESC LIMIT 1";
        try { return jdbcTemplate.queryForObject(sql, String.class, memberId); } 
    ...
    public String findActivityPattern(String memberId) {
        String sql = "SELECT CASE DAYOFWEEK(created_at) WHEN 1 THEN '일요일' WHEN 2 THEN '월요일' WHEN 3 THEN '화요일' WHEN 4 THEN '수요일' WHEN 5 THEN '목요일' WHEN 6 THEN '금요일' WHEN 7 THEN '토요일' END as day_of_week FROM user_reviews WHERE member_id = ? GROUP BY day_of_week ORDER BY COUNT(*) DESC LIMIT 1";
        try { return jdbcTemplate.queryForObject(sql, String.class, memberId); } 
        catch (org.springframework.dao.EmptyResultDataAccessException e) { return null; }
    }

    public Map<String, Double> findMoviePreferenceMeta(String memberId) {
        String sql = "SELECT AVG(m.year) as avg_year, AVG(CAST(REGEXP_REPLACE(m.runtime, '[^0-9]+', '') AS UNSIGNED)) as avg_runtime FROM user_reviews r JOIN movies m ON r.movie_id = m.id WHERE r.member_id = ? AND r.user_rating >= ?";
        try {
            return jdbcTemplate.queryForObject(sql, (rs, rowNum) -> {
                ...
                if (avgYearObj != null) meta.put("avgYear", ((Number) avgYearObj).doubleValue());
                if (avgRuntimeObj != null) meta.put("avgRuntime", ((Number) avgRuntimeObj).doubleValue());
                return meta;

...
<%-- 07-31: home.jsp에 include되므로, <html>, <head>, <body> 태그를 포함하지 않습니다. --%>
<%-- 07-31: 모든 <link> 태그와 <style> 태그는 home.jsp의 <head>로 옮기는 것이 좋습니다. --%>
                            ...
                            <c:choose>
                                <c:when test="${not empty movie.posterPath and movie.posterPath ne 'N/A'}">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <c:url value='${movie.posterPath}'/>
                                        </c:otherwise>
                                    </c:choose>
                                </c:when>
                                <c:otherwise>
                                    <c:url value='/resources/images/movies/256px-No-Image-Placeholder.png'/>
                                </c:otherwise>
                            </c:choose>
                        </c:set>
...
<%-- 07-31: JavaScript 코드 (home.jsp의 </body> 바로 위에 위치 권장) --%>
<%-- home.jsp에 jQuery 포함되어 있다고 가정 --%>
<script>

...
1. 등급 미정 보정 흐름을 정리한다. TMDB에서 등급을 못 가져온 영화는 OMDb 같은 보조 조회를 붙여서 관리자 보정 단계에서 채운다.
2. 영화 상세 페이지의 무거운 조회를 나눈다. 추천 영화, 배우/감독, 이미지 갤러리처럼 첫 화면에 바로 필요하지 않은 정보는 늦게 불러온다.
3. 검색 필터를 정리한다. 검색창 아래에서 포스터 가리기와 장르/등급 제외 조건을 고를 수 있게 하고, 검색어 자체와 필터 조건은 섞이지 않게 한다.
4. 홈 화면에서 사이트 특징을 더 분명히 보여준다. 네 가지 평가지수와 취향 분석으로 이어지는 흐름을 자연스럽게 배치한다.
5. 영화 상세 페이지에 부가 정보를 추가한다. 명대사, 시청 가능 서비스는 상세 페이지를 무겁게 만들지 않도록 별도 영역으로 늦게 불러오는 방식을 검토한다.
...
- 영화 상세 페이지에 명대사 영역을 추가하는 기능을 검토한다. 처음에는 관리자가 영화별로 직접 등록하는 방식이 가장 단순하다.
- 영화 상세 페이지에 시청 가능 서비스 영역을 추가하는 기능을 검토한다. TMDB 제공처 정보를 우선 사용하고, 화면 렌더링이 느려지지 않게 상세 페이지 진입 후 따로 불러오는 방식이 적당하다.
